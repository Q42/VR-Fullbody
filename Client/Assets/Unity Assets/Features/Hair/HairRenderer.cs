...
public class HairRenderer : MonoBehaviour {
	...
	void CreateMaterials() {
		...
		m_materialOpaque.name = sourceMat.name +  " (hair opaque)";
		...
		m_materialBack.name = sourceMat.name + " (hair back)";
		...
		m_materialFront.name = sourceMat.name + " (hair front)";
	...
	void UpdateMaterials() {
		...
		m_materialFront.SetFloat("_Cutoff", frontAlphaRef);
		if(frontAlphaRef > 0f)
	...
	void SelectMesh() {
		...
		if(useOpaquePass)
			AddChild(m_materialOpaque);
		AddChild(m_materialBack);
		AddChild(m_materialFront);
	}
	...
	void OnValidate() {
		if(m_sourceMesh) {
			...
			frontAlphaRef = Mathf.Clamp(frontAlphaRef, 0f, 1.001f);
